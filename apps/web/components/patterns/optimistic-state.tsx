...
export function OptimisticStateIndicator({ state = "synced" }: { state?: SyncState }) {
  ...
  if (state === "pending") return <span className="inline-flex items-center gap-1 text-xs text-[var(--text-secondary)]"><LoaderCircle className="size-3 animate-spin" />Saving</span>;
