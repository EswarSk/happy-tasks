...
export function TaskDetailPanel({ projectId, taskId, members, onClose, onOpenTask }: TaskDetailPanelProps) {
  ...
  if (query.isLoading || !task) {
    return <div className="h-full bg-[var(--panel)] p-5"><div className="h-8 w-2/3 animate-pulse rounded bg-slate-100" /><div className="mt-6 space-y-3">{Array.from({ length: 6 }).map((_, index) => <div key={index} className="h-12 animate-pulse rounded-lg bg-slate-100" />)}</div></div>;
  }
  ...
  return (
    <section className="flex h-full min-h-0 flex-col bg-[var(--panel)]" aria-label={`Task details for ${task.title}`}>
      <header className="shrink-0 border-b border-[var(--border)] px-5 pt-4 pb-3">
        ...
        <Input value={title} onChange={(event) => setDraft({ taskId: task.id, title: event.target.value, description })} className="h-auto border-0 px-0 py-1 text-lg font-semibold shadow-none focus:ring-0" aria-label="Task title" />
      </header>
            ...
            <div className="mt-3 grid grid-cols-[112px_minmax(0,1fr)] items-center gap-y-3 text-sm">
              ...
              <div className="flex flex-wrap gap-1.5">
                {members.map((member) => {
                  ...
                  return <button key={member.id} aria-pressed={selected} onClick={() => update.mutate({ assigneeIds: selected ? task.assigneeIds.filter((id) => id !== member.id) : [...task.assigneeIds, member.id], expectedVersion: task.version })} className={cn("flex items-center gap-1.5 rounded-full border py-1 pr-2 pl-1 text-xs", selected ? "border-indigo-200 bg-indigo-50 text-indigo-800" : "border-[var(--border)] text-[var(--text-muted)] hover:bg-[var(--hover)]")}><Avatar name={member.displayName} color={member.color} className="size-5 border-0" />{member.displayName.split(" ")[0]}</button>;
                })}
              </div>
              <span className="text-[var(--text-muted)]">Tags</span>
              <div className="space-y-2">
                <div className="flex flex-wrap gap-1.5">
                  {task.tags.map((tag) => <Badge key={tag} className="border-slate-200 bg-slate-50 text-slate-600">{tag}<button type="button" aria-label={`Remove ${tag} tag`} disabled={update.isPending} onClick={() => update.mutate({ tags: task.tags.filter((item) => item !== tag), expectedVersion: task.version })} className="rounded-sm text-slate-400 hover:text-rose-600 focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-[var(--focus)]"><X className="size-3" /></button></Badge>)}
                </div>
              ...
              <div className="flex flex-wrap gap-1.5">{Object.entries(task.customFields).length ? Object.entries(task.customFields).map(([key, value]) => <Badge key={key} className="border-slate-200 bg-white text-slate-600"><span className="text-slate-400">{key}</span>{String(value)}</Badge>) : <span className="text-xs text-[var(--text-muted)]">None configured</span>}</div>
            </div>
          </section>
          ...
          <section aria-labelledby="dependencies-heading">
            ...
            <div className="mt-3 space-y-2">
              {task.dependencyIds.map((id) => {
                ...
                return <div key={id} className="flex w-full items-center gap-1 rounded-lg border border-[var(--border)] pr-1"><button onClick={() => onOpenTask(id)} className="flex min-w-0 flex-1 items-center gap-2 px-3 py-2 text-left text-xs hover:bg-[var(--hover)]"><Check className="size-3.5 text-emerald-600" /><span className="min-w-0 flex-1 truncate">{linked?.title ?? id.slice(0, 8)}</span><ChevronRight className="size-3.5 text-[var(--text-muted)]" /></button><Button variant="ghost" size="icon" className="size-7" aria-label={`Remove dependency ${linked?.title ?? id}`} disabled={removeDependency.isPending} onClick={() => removeDependency.mutate(id)}><X className="size-3.5" /></Button></div>;
              })}
              ...
              {dependencyError && <p className="flex items-start gap-1.5 text-xs text-rose-700"><AlertTriangle className="mt-0.5 size-3.5 shrink-0" />{dependencyError}</p>}
              <p className="text-[10px] text-[var(--text-muted)]">Cycle-forming dependency edges are rejected transactionally.</p>
            </div>
          </section>
        </div>

        <Tabs.Root defaultValue="comments" className="flex min-h-[390px] flex-col border-t border-[var(--border)]">
          <Tabs.List className="flex h-11 shrink-0 gap-5 border-b border-[var(--border)] px-5" aria-label="Task collaboration">
            <Tabs.Trigger value="comments" className="border-b-2 border-transparent text-xs font-semibold text-[var(--text-muted)] outline-none data-[state=active]:border-[var(--brand)] data-[state=active]:text-[var(--text)]"><MessageSquare className="mr-1.5 inline size-3.5" />Comments <span className="ml-1 rounded-full bg-slate-100 px-1.5">{task.commentCount}</span></Tabs.Trigger>
          ...
          <Tabs.Content value="activity" className="p-5 outline-none">
            {[
              ["Task moved to In progress", "2026-08-26T21:05:00Z"],
              ["Maya added a dependency", "2026-08-26T19:42:00Z"],
              ["Description was updated", "2026-08-26T17:18:00Z"],
            ].map(([item, occurredAt], index) => <div key={item} className="relative flex gap-3 pb-5 text-xs before:absolute before:top-6 before:bottom-0 before:left-3 before:w-px before:bg-[var(--border)] last:before:hidden"><Avatar name={members[index]?.displayName ?? "Team"} color={members[index]?.color} className="size-6" /><div><p className="text-[var(--text-secondary)]">{item}</p><p className="mt-1 text-[var(--text-muted)]">{relativeTime(occurredAt)}</p></div></div>)}
          </Tabs.Content>
        </Tabs.Root>
      </div>
      ...
      <Dialog open={conflictOpen} onOpenChange={setConflictOpen} title="A newer version is available" description="This task changed on another client while you were editing. Review the latest server state before applying your changes again.">
        <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900"><AlertTriangle className="mr-2 inline size-4" />Server version {task.version + 1} is newer than your edit.</div>
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen} title="Delete this task?" description="The task, its comments, assignments, tags, and dependency edges will be removed in one transaction. This action cannot be undone.">
        <div className="rounded-lg border border-rose-200 bg-rose-50 p-3 text-sm text-rose-900"><Trash2 className="mr-2 inline size-4" />Delete {task.key} · {task.title}</div>
